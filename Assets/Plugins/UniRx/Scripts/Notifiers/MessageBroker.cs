...
namespace EranCore.UniRx
{
    ...
    public class AsyncMessageBroker : IAsyncMessageBroker, IDisposable
    {
        ...
        {
            EranCore.UniRx.InternalUtil.ImmutableList<Func<T, IObservable<Unit>>> notifier;
            lock (notifiers)
            {
                ...
                {
                    notifier = (EranCore.UniRx.InternalUtil.ImmutableList<Func<T, IObservable<Unit>>>)_notifier;
                }
                else
                {
        ...
        public IDisposable Subscribe<T>(Func<T, IObservable<Unit>> asyncMessageReceiver)
        {
            lock (notifiers)
            {
                ...
                if (!notifiers.TryGetValue(typeof(T), out _notifier))
                {
                    var notifier = EranCore.UniRx.InternalUtil.ImmutableList<Func<T, IObservable<Unit>>>.Empty;
